...
## 출처 표기 확정 문구 (2026-08-21, 앱 설정 화면 적용)

> 식품의약품안전처 「조리식품의 레시피 DB」 (공공데이터포털 15060073) — 요리 검색·재료 매핑
> 식품의약품안전처 「식품영양성분DB」 (공공데이터포털 15127578) — 식재료 마스터 구축 참고

스토어 설명·개인정보처리방침에도 같은 문구가 필요한지는 배포 회차에 이용 조건 재확인. 문구 수정 시 이 절이 원본.
