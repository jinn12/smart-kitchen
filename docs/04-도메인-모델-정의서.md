...
구현 규칙 (2026-08-20 확정, D-027):

- 확정 대상: `status = PLANNED AND plan_date < 오늘` (CANCELED·CONFIRMED·오늘 이후 계획은 제외)
- 차감량 = **min(meal_plan_item.reserved_qty, 현재 실물 잔량)** — 등록 후 재고가 줄었을 수 있어 실물보다 많이 깎지 않는다. reserved_quantity는 스냅샷 전액 해제(차감량과 별개)
- FEFO(R-2)로 배치를 순회해 깎고, **실제 차감량**으로 InventoryHistory(CONSUME, ref_type=MEAL_PLAN, ref_id=계획 id) 기록
- **계획 단위 트랜잭션** — 한 건 실패가 다른 계획을 막지 않는다
- 재실행 안전(멱등): 대상 쿼리가 CONFIRMED를 거르고, 확정 로직에도 status 가드를 이중으로 둔다
- 임박 알림의 판정은 재고 화면(API-24)과 같은 로직을 공유한다 — 화면과 푸시의 기준이 어긋나지 않도록. 항목 있는 가구만 발송(D-013). 푸시 발송은 NotificationSender 인터페이스 뒤에 두고, FCM 연동은 앱 개발 후
